...
func New(vp *viper.Viper) *cobra.Command {
	recordCmd := &cobra.Command{
		...
		Long:  "Capture and record network packets into a pcap file stored on the Triangle server",
		Example: `
Record all TCP traffic from 192.168.1.0/24 (any source port) to 10.0.0.0/16 port 80:

  triangle record "192.168.1.0/24 0 10.0.0.0/16 80 TCP"
...
protocols are TCP, UDP, SCTP, and ANY.`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return runRecord(ctx, triangleConn, filters)
		},
	}

	recorderFlags := pflag.NewFlagSet("recorder", pflag.ContinueOnError)
	recorderFlags.StringVar(&fileSinkPrefix, "file-prefix", "triangle", "File prefix of the resulting pcap file")
