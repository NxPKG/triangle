...
func TestFromToWorkloadCombined(t *testing.T) {
	t.Run("single filter", func(t *testing.T) {
		...
		require.NoError(t, cmd.Flags().Parse([]string{"--from-pod", "shipyard", "--to-workload", "app"}))
		if diff := cmp.Diff(
			[]*flowpb.FlowFilter{
				{
					SourcePod:           []string{"shipyard"},
	...
	t.Run("two filters", func(t *testing.T) {
		...
		require.NoError(t, cmd.Flags().Parse([]string{"--pod", "shipyard", "--to-workload", "app"}))
		if diff := cmp.Diff(
			[]*flowpb.FlowFilter{
				{
					SourcePod:           []string{"shipyard"},
				...
				{
					DestinationPod:      []string{"shipyard"},
					DestinationWorkload: []*flowpb.Workload{{Name: "app"}},
				},
			},
			f.whitelist.flowFilters(),
...
func TestFromToIdentityCombined(t *testing.T) {
	t.Run("single filter", func(t *testing.T) {
		...
		require.NoError(t, cmd.Flags().Parse([]string{"--from-pod", "shipyard", "--to-identity", "42"}))
		if diff := cmp.Diff(
			[]*flowpb.FlowFilter{
				{SourcePod: []string{"shipyard"}, DestinationIdentity: []uint32{42}},
			},
			f.whitelist.flowFilters(),
			cmpopts.IgnoreUnexported(flowpb.FlowFilter{}),
		); diff != "" {
	...
	t.Run("two filters", func(t *testing.T) {
		...
		require.NoError(t, cmd.Flags().Parse([]string{"--pod", "shipyard", "--to-identity", "42"}))
		if diff := cmp.Diff(
			[]*flowpb.FlowFilter{
				{SourcePod: []string{"shipyard"}, DestinationIdentity: []uint32{42}},
				{DestinationPod: []string{"shipyard"}, DestinationIdentity: []uint32{42}},
			},
			f.whitelist.flowFilters(),
			cmpopts.IgnoreUnexported(flowpb.FlowFilter{}),
		); diff != "" {
...
func TestHTTPURL(t *testing.T) {
	...
	require.NoError(t, cmd.Flags().Parse([]string{"--http-url", `http://.*shipyard\.io/foo`, "--http-url", `http://www\.shipyard\.io/bar`}))
	if diff := cmp.Diff(
		[]*flowpb.FlowFilter{
			{HttpUrl: []string{`http://.*shipyard\.io/foo`, `http://www\.shipyard\.io/bar`}},
		},
		f.whitelist.flowFilters(),
		cmpopts.IgnoreUnexported(flowpb.FlowFilter{}),
	); diff != "" {
