...
// see protocol filter in Triangle server code (there is unfortunately no
...
// types are not supported by `triangle observe flows` but have separate
// sub-commands.
...
// passed to triangle observe command via `--config` flag.
...
// GetTriangleClientFunc is primarily used to mock out the triangle client in some unit tests.
var GetTriangleClientFunc = func(ctx context.Context, vp *viper.Viper) (client observerpb.ObserverClient, cleanup func() error, err error) {
	...
	// read flows from a triangle server
	triangleConn, err := conn.New(ctx, vp.GetString(config.KeyServer), vp.GetDuration(config.KeyTimeout))
	if err != nil {
		return nil, nil, err
	}
	logger.Logger.Debug("connected to Triangle API", "server", config.KeyServer)
	cleanup = triangleConn.Close
...
func newObserveCmd(vp *viper.Viper) *cobra.Command {
	...
	usage := cmdUsage{
		use:   "observe",
		short: "Observe flows and events of a Triangle server",
	}
...
func newFlowsCmdWithFilter(vp *viper.Viper, ofilter *flowFilter) *cobra.Command {
	usage := cmdUsage{
		example: `* Piping flows to triangle observe

  Save output from 'triangle observe -o jsonpb' command to a file, and pipe it to
  the observe command later for offline processing. For example:

    triangle observe -o jsonpb --last 1000 > flows.json

  Then,

    cat flows.json | triangle observe --input-file -
...
* Filtering flows
  ...
  command shows all flows that use the HTTP PUT method.
    ...
    triangle observe --http-method put
  ...
  using either a PUT or GET method.
    ...
    triangle observe --http-method put --http-method get
  ...
  returned.
    ...
    triangle observe --http-method put --http-method get --to-namespace foo
  ...
  requests to endpoints in namespace 'foo' or 'bar'
    ...
    triangle observe --http-method put --http-method get --to-namespace foo --to-namespace bar
...
* Using negations on filters
  ...
  method.
    ...
    triangle observe --not --http-method put
  ...
  GET nor PUT method.
    ...
    triangle observe --not --http-method put --not --http-method get
  ...
  don't use a GET or PUT method to endpoints in the 'foo' namespace.
    ...
    triangle observe --not --http-method put --not --http-method get --not --to-namespace foo
  ...
  `,
		use:   "flows",
		short: "Observe flows of a Triangle server",
...
func newFlowsCmdHelper(usage cmdUsage, vp *viper.Viper, ofilter *flowFilter) *cobra.Command {
	...
	flowsCmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			client, cleanup, err := GetTriangleClientFunc(ctx, vp)
			if err != nil {
				return err
			}
			defer cleanup()
	...
	filterFlags.Var(filterVar(
		"http-url", ofilter,
		`Show only flows which match this HTTP URL regular expressions (e.g. "http://.*shipyard\.io/page/\\d+")`))
	...
	filterFlags.Var(filterVar(
		"from-fqdn", ofilter,
		`Show all flows originating at the given fully qualified domain name (e.g. "*.khulnasoft.com").`))
	filterFlags.Var(filterVar(
		"fqdn", ofilter,
		`Show all flows related to the given fully qualified domain name (e.g. "*.khulnasoft.com").`))
	filterFlags.Var(filterVar(
		"to-fqdn", ofilter,
		`Show all flows terminating at the given fully qualified domain name (e.g. "*.khulnasoft.com").`))
...
func getFlowsRequest(ofilter *flowFilter, allowlist []string, denylist []string) (*observerpb.GetFlowsRequest, error) {
	...
	if since == nil && until == nil && !first {
		...
		case selectorOpts.last == 0 && !selectorOpts.follow && otherOpts.inputFile == "":
			...
			// `triangle observe` in non-follow mode
