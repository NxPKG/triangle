...
var (
	...
	// generic formatting flags, available to `triangle observe`, including sub-commands.
...
func init() {
	selectorFlags.BoolVar(&selectorOpts.all, "all", false, "Get all flows stored in Triangle's buffer. Note: this option may cause Triangle to return a lot of data. It is recommended to only use it along filters to limit the amount of data returned.")
	selectorFlags.Uint64Var(&selectorOpts.last, "last", 0, fmt.Sprintf("Get last N flows stored in Triangle's buffer (default %d). When querying against Triangle Relay, this gets N flows per instance of Triangle connected to that Relay.", defaults.FlowPrintCount))
	selectorFlags.Uint64Var(&selectorOpts.first, "first", 0, "Get first N flows stored in Triangle's buffer. When querying against Triangle Relay, this gets N flows per instance of Triangle connected to that Relay.")
...
`)
 ...
 `,
	...
	otherFlags.BoolVar(&otherOpts.printRawFilters,
		...
		"Print allowlist/denylist filters and exit without sending the request to Triangle server")
