...
const (
	...
	// FlowPrintCount is the default number of flows to print on the triangle
	// observe CLI.
	FlowPrintCount = 20

	// EventsPrintCount is the default number of agent/debug events to print
	// on the triangle events CLI.
	EventsPrintCount = 20
	...
	socketPathKey = "TRIANGLE_DEFAULT_SOCKET_PATH"

	// socketPath is the path of the socket on which to connect to the local
	// triangle observer. Use GetDefaultSocketPath to access it.
	socketPath = "unix:///var/run/khulnasoft/triangle.sock"
)

var (
	// ConfigDir is the default directory path to store Triangle
	// configuration files. It may be unset.
	ConfigDir string
	// ConfigDirFallback is the directory path to store Triangle configuration
...
func init() {
	...
	if dir, err := os.UserConfigDir(); err == nil {
		ConfigDir = filepath.Join(dir, "triangle")
	}
	// fallback to home directory
	if dir, err := os.UserHomeDir(); err == nil {
		ConfigDirFallback = filepath.Join(dir, ".triangle")
	}
